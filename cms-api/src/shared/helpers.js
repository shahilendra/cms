...
exports.updateBatch = function(data, body, createdBy, systemSetting) {
  if(systemSetting == null) {
    return Promise.reject({ message: 'Session is not be empty. So please setup default session in system settings!'});
  }
  if(data == null) {
    return Promise.reject({ message: 'Batch id is not be empty. So please enter valid batch id!'});
  } else  if(body.name) {
    let dataMap = {};
    if(data.id) {
      dataMap.updatedBy = createdBy;
    ...
    return Promise.reject({ message: 'Batch name should not be empty. So please enter valid batch name!'});
  }
}

exports.updateSession = function(data, body, createdBy) {
  if(data == null) {
    return Promise.reject({ message: 'Session id is not be empty. So please enter valid session id!'});
  } else  if(body.name) {
    let dataMap = {};
    if(data.id) {
      dataMap.updatedBy = createdBy;
    ...
    dataMap.name = body.name;
    dataMap.startDate = body.startDate;
    dataMap.endDate = body.endDate;
    dataMap.isActive = body.isActive;
    dataMap.organisationId = body.organisationId;
    return Promise.resolve(dataMap);
  } else {
    return Promise.reject({ message: 'Session name should not be empty. So please enter valid session name!'});
  }
}

exports.updateSystemSetting = function(data, body, createdBy) {
  if(data == null) {
    return Promise.reject({ message: 'System Setting id is not be empty. So please enter valid System Setting id!'});
    ...
    dataMap.email = body.email;
    dataMap.isActive = body.isActive;
    dataMap.organisationId = body.organisationId;
    return Promise.resolve(dataMap);
  } else {
    return Promise.reject({ message: 'Session Id should not be empty. So please enter valid Session Id!'});
  }
}

exports.updateStudent = function(data, body, createdBy, systemSetting) {
  if(systemSetting == null) {
    return Promise.reject({ message: 'Session is not be empty. So please setup default session in system settings!'});
  }
  if(data == null) {
    return Promise.reject({ message: 'Student id is not be empty. So please enter valid Student id!'});
  } else  if(body.rollNumber) {
    let dataMap = {};
    if(data.id) {
      dataMap.updatedBy = createdBy;
    } else {
    ...
    dataMap.rollNumber = body.rollNumber;
    dataMap.firstName = body.firstName;
    dataMap.lastName = body.lastName;
    dataMap.middleName = body.middleName;
    dataMap.fatherName = body.fatherName;
    dataMap.motherName = body.motherName;
    dataMap.phone = body.phone;
    dataMap.parentPhone = body.parentPhone;
    dataMap.address = body.address;
